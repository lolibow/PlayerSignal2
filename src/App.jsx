...
export default function App() {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      if (results.length === 0) {
        setError('No games found. Try a different title.');
      }
    } catch (e) {
      setError('Failed to search games. Please check your connection.');
      console.error(e);
    } finally {
      setSearching(false);
    }
        ...
        <aside style={styles.aside}>
          {searching && (
            <div style={styles.scanMsg}>
              ...
              <span style={styles.scanText}>SCANNING DATABASE...</span>
            </div>
          )}
          ...
          {!hasSearched && (
            <div style={styles.welcomePanel}>
              ...
              <div style={styles.welcomeTitle}>READY FOR ANALYSIS</div>
              <div style={styles.welcomeText}>
                Search for any game title to begin sentiment intelligence scanning.
                Designed for gaming journalists and editors.
              </div>
              <div style={styles.welcomeFeatures}>
                <Feature icon="◆" text="Sentiment scoring (0–100)" />
                <Feature icon="◆" text="Positive/negative signal detection" />
                <Feature icon="◆" text="Per-review breakdown" />
                <Feature icon="◆" text="Community distribution analysis" />
              </div>
            </div>
          )}
            ...
            <div style={styles.mainPlaceholder}>
              ...
              <div style={styles.placeholderMsg}>
                ...
                <span style={styles.placeholderText}>
                  {hasSearched ? 'SELECT A GAME TO ANALYZE' : 'AWAITING INPUT'}
                </span>
              </div>
            </div>
          )}
